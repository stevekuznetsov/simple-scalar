...
/* branch predictor type {nottaken|taken|perfect|bimod|2lev|1bit|3bit|4bit|5bit|6bit|percept|gehl} */
                  ...
                  int argc, char **argv)        /* command line arguments */
{
  ...
  else if (!mystricmp(pred_type, "4bit"))
    {
      /* 4bit predictor, bpred_create() checks BTB_SIZE */
      if (bimod_nelt != 1)
        fatal("bad 4bit predictor config (<table_size>)");
      if (btb_nelt != 2)
        fatal("bad btb config (<num_sets> <associativity>)");

      /* bimodal predictor, bpred_create() checks BTB_SIZE */
      pred = bpred_create(BPred4bit,
                          /* bimod table size */bimod_config[0],
                          /* 2lev l1 size */0,
                          /* 2lev l2 size */0,
                          /* meta table size */0,
  ...
  else if (!mystricmp(pred_type, "5bit"))
    {
      /* 5bit predictor, bpred_create() checks BTB_SIZE */
      if (bimod_nelt != 1)
        fatal("bad 4bit predictor config (<table_size>)");
      if (btb_nelt != 2)
        fatal("bad btb config (<num_sets> <associativity>)");
  ...
  else if (!mystricmp(pred_type, "6bit"))
    {
      /* 6bit predictor, bpred_create() checks BTB_SIZE */
      if (bimod_nelt != 1)
        fatal("bad 4bit predictor config (<table_size>)");
      if (btb_nelt != 2)
        fatal("bad btb config (<num_sets> <associativity>)");
  ...
  else if (!mystricmp(pred_type, "gehl"))
    {
      /* gehl predictor, bpred_create() checks args */
      if (twolev_nelt != 4)
        fatal("bad gehl pred config (<l1size> <l2size> <hist_size> <xor>)");
      if (btb_nelt != 2)
        fatal("bad btb config (<num_sets> <associativity>)");

      pred = bpred_create(BPredGehl,
                          /* bimod table size */0,
                          /* gehl l1 history table size */twolev_config[0],
                          /* gehl l2 table size */twolev_config[1],
                          /* meta table size */0,
                          /* history reg size */twolev_config[2],
                          /* history xor address */twolev_config[3],
                          /* btb sets */btb_config[0],
